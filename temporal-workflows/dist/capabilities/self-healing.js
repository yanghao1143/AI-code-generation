...
exports.autoFixProblems = autoFixProblems;
exports.runSelfHealing = runSelfHealing;
const child_process_1 = require("child_process");
const util_1 = require("util");
const execAsync = (0, util_1.promisify)(child_process_1.exec);
// 问题检测规则
const PROBLEM_DETECTORS = [
    {
        ...
        detect: async () => {
            ...
            for (const agent of agents) {
                try {
                    const { stdout } = await execAsync(`tmux -S /tmp/openclaw-agents.sock capture-pane -t ${agent} -p 2>/dev/null | tail -10`);
    ...
    },
    {
        ...
        detect: async () => {
            ...
            for (const agent of agents) {
                try {
                    const { stdout } = await execAsync(`tmux -S /tmp/openclaw-agents.sock capture-pane -t ${agent} -p 2>/dev/null | wc -l`);
...
async function autoFixProblems() {
    const results = [];
    for (const detector of PROBLEM_DETECTORS) {
        try {
            const problem = await detector.detect();
