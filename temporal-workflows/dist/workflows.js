...
async function fullInspectionWorkflow() {
    ...
    if (typescript.hasErrors || build.hasErrors) {
        await addMemory(`巡检发现问题: TS错误${typescript.errorCount}个, 构建${build.hasErrors ? '失败' : '成功'}`, 'inspection', 8);
    }
    return {
        health,
...
async function learningEvolutionWorkflow() {
    ...
    if (importantInsights.length > 0) {
        ...
        if (shouldNotify) {
            await sendNotification(`发现 ${importantInsights.length} 个重复模式需要关注`);
        }
    }
...
async function systemMonitorWorkflow() {
    ...
    const issues = [];
    if (!health.redis)
        issues.push('Redis 不可用');
    if (!health.postgres)
        issues.push('PostgreSQL 不可用');
    if (!health.temporal)
        issues.push('Temporal 不可用');
    if (health.diskUsage > 90)
        issues.push(`磁盘使用率过高: ${health.diskUsage}%`);
    if (health.memoryUsage > 90)
        issues.push(`内存使用率过高: ${health.memoryUsage}%`);
    // 如果有严重问题，发送告警
    if (issues.length > 0) {
...
async function directorDailyReviewWorkflow() {
    ...
    await addMemory(`技术总监巡检: ${projectHealths.length}个项目, ${issues.length}个问题, ${actions.length}个行动`, 'director-review', 7);
...
async function selfHealingWorkflow() {
    ...
    if (result.problemsFound > 0) {
        await addMemory(`自愈运行: 发现${result.problemsFound}个问题, 修复${result.problemsFixed}个`, 'self-healing', result.problemsFixed < result.problemsFound ? 8 : 5);
    }
    return {
...
async function deepLearningWorkflow() {
    ...
    if (result.knowledgeSaved > 0) {
        await addMemory(`深度学习: 发现${result.patternsFound}个模式, 生成${result.insightsGenerated}个洞察, 保存${result.knowledgeSaved}条知识`, 'deep-learning', 7);
    }
    return result;
}
