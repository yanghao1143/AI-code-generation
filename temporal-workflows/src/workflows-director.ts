...
export async function directorDailyReviewWorkflow(): Promise<{
  ...
  // 3. 记录巡检结果
  await addMemory(
    `技术总监日常巡检: ${projectHealths.length}个项目, ${issues.length}个问题, ${actions.length}个行动`,
    'director-review',
    7
  );
 ...
 * 根据任务内容选择最佳 Agent
 */
export async function intelligentTaskAssignmentWorkflow(
  task: string,
  ...
  if (success) {
    await addMemory(
      `任务分配: "${task.slice(0, 50)}..." -> ${assignment.agent} (${assignment.reason})`,
      'task-assignment',
      priority === 'high' ? 8 : 5
    );
  }
 ...
 * 技术决策记录
 */
export async function recordTechDecisionWorkflow(
  title: string,
  context: string,
  decision: string,
  consequences: string
 ...
 * 周报生成
 */
export async function weeklyReportWorkflow(): Promise<string> {
  const lines: string[] = ['📊 本周技术总监周报\n'];
