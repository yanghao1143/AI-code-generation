...
use settings::{AnthropicSettingsContent, RegisterSetting, Settings, SettingsContent, update_settings_file};
use std::{
    any::TypeId,
    convert::TryFrom,
    fmt::Write as _,
...
pub static ZED_OAUTH_CLIENT_ID: LazyLock<String> =
    LazyLock::new(|| std::env::var("ZED_OAUTH_CLIENT_ID").unwrap_or_else(|_| "zed".to_string()));

/// Storage for Chi Code OAuth provider credentials
/// After OAuth login, this stores the base_url and api_key for configuring editor providers
pub static CHICODE_OAUTH_PROVIDER_CREDS: LazyLock<parking_lot::Mutex<Option<ChiCodeProviderCredentials>>> =
    LazyLock::new(|| parking_lot::Mutex::new(None));

/// Chi Code OAuth provider credentials for configuring editor's language model providers
#[derive(Clone, Debug)]
pub struct ChiCodeProviderCredentials {
    /// The base URL for API calls (e.g., https://vip.chiddns.com)
    pub base_url: String,
...
impl Client {
                    ...
                    match authenticate {
                        Ok(creds) => {
                            if IMPERSONATE_LOGIN.is_none() {
                                ...
                                // Configure Anthropic provider with Chi Code OAuth credentials
                                if let Some(chicode_creds) = CHICODE_OAUTH_PROVIDER_CREDS.lock().clone() {
                                    log::info!("[OAuth] Configuring Anthropic provider with Chi Code credentials");
                                    // Write API key to credential provider for Anthropic
                                    // The URL should match what AnthropicLanguageModelProvider uses
                                    let anthropic_url = chicode_creds.base_url.clone();
                                    let api_key = chicode_creds.api_key.clone();

                                    // Log API key info (partial, for debugging)
                                    let key_preview = if api_key.len() > 10 {
                                        format!("{}...{} (len={})", &api_key[..5], &api_key[api_key.len()-5..], api_key.len())
                                    } else {
                                        format!("{} (len={})", &api_key, api_key.len())
                                    };
                                    log::info!("[OAuth] Storing API key: {} for URL: {}", key_preview, anthropic_url);
                                    ...
                                    if let Err(e) = task.await {
                                        log::error!("[OAuth] Failed to write Anthropic credentials: {:?}", e);
                                    } else {
                                        log::info!("[OAuth] Anthropic credentials written successfully for URL: {}", anthropic_url);
                                    }

                                    // Update Anthropic settings to use Chi Code's base URL
                                    let url_for_settings = anthropic_url.clone();
                                    cx.update(|cx| {
                                        ...
                                        update_settings_file(fs, cx, |settings, _cx| {
                                            ...
                                            match &mut language_models.anthropic {
                                                Some(anthropic) => {
                                                    anthropic.api_url = Some(url_for_settings);
                                                }
                                                None => {
                                                    language_models.anthropic = Some(AnthropicSettingsContent {
                                                        api_url: Some(url_for_settings),
                                                        api_key: None,
                                                        use_cli: None,
                                        ...
                                        log::info!("[OAuth] Anthropic api_url setting updated");
                                    });
                                }
                            }
    ...
    /// 生成 PKCE 随机字符串
    fn generate_pkce_random_string(length: usize) -> String {
    ...
    /// 判断是否使用自定义 OAuth 服务器（非 zed.dev）
    fn is_custom_oauth_server(server_url: &str) -> bool {
        !server_url.contains("zed.dev")
    }
    ...
    pub fn authenticate_with_browser(self: &Arc<Self>, cx: &AsyncApp) -> Task<Result<Credentials>> {
        ...
        cx.spawn(async move |cx| {
            ...
            // 获取服务器 URL 来决定使用哪种认证方式
            let server_url = http.base_url().to_string();
            let use_oauth_pkce = Self::is_custom_oauth_server(&server_url);

            let credentials = if use_oauth_pkce {
                // ========== Standard OAuth2 PKCE Flow (Custom Server) ==========
                log::info!("[OAuth] Using Standard OAuth2 PKCE for server: {}", server_url);

                background
                    .clone()
                    .spawn(async move {
                        // 1. Generate PKCE params
                        let code_verifier = Self::generate_pkce_random_string(64);
                        let code_challenge = Self::generate_pkce_code_challenge(&code_verifier);
                        let state = Self::generate_pkce_random_string(32);

                        // 2. Start local callback server
                        let server = tiny_http::Server::http(format!("127.0.0.1:{}", Self::OAUTH_CALLBACK_PORT))
                            .map_err(|e| anyhow!("Failed to start callback server: {}", e))?;

                        // 3. Construct Authorization URL (Chi Code Local App OAuth)
                        // Uses same endpoint as cc app: /login?oauth_local_app=true
                        let login_url = format!(
                            "{}/login?oauth_local_app=true&state={}&code_challenge={}&redirect_port={}",
                        ...
                        log::info!("[OAuth] Opening browser: {}", login_url);
                        open_url_tx.send(login_url).log_err();
                        ...
                        let (received_code, received_state) = background
                            .spawn(async move {
                                for _ in 0..300 { // 5 minutes timeout
                                    if let Some(req) = server.recv_timeout(Duration::from_secs(1))? {
                                        ...
                                        if let Some(error) = params.error {
                                            let _ = req.respond(tiny_http::Response::from_string(format!("Auth failed: {}", error)));
                                        ...
                                        match (params.code, params.state) {
                                            (Some(code), Some(state)) => {
                                                let _ = req.respond(tiny_http::Response::from_string(
                                                    "<h1>Login Successful</h1><p>You can close this window.</p><script>window.close()</script>"
                                                ).with_header(tiny_http::Header::from_bytes(&b"Content-Type"[..], &b"text/html"[..]).unwrap()));
                                                return Ok((code, state));
                                            }
                                            _ => {
                                                let _ = req.respond(tiny_http::Response::from_string("Missing code or state"));
                                                return Err(anyhow!("Missing code/state"));
                                            }
                                        }
                                    }
                                }
                        ...
                        // 5. Exchange Code for Token (Chi Code Local App OAuth)
                        // Uses same endpoint as cc app: /api/oauth/local-app/token
                        let token_url = format!("{}/api/oauth/local-app/token", server_url.trim_end_matches('/'));
                        ...
                        log::info!("[OAuth] Token exchange response status: {}", response.status());
                        log::debug!("[OAuth] Token exchange raw response: {}", body);

                        if !response.status().is_success() {
                            return Err(anyhow!("Token exchange failed: {} - {}", response.status(), body));
                        }

                        // Parse Chi Code Local App OAuth response
                        // Response format: { success: bool, message: String, data: { access_token, base_url, api_key, user: { id, username, display_name, role, group } } }
                        #[derive(Deserialize)]
                        struct ChiCodeAuthResponse {
                            success: bool,
                            message: String,
                            data: Option<ChiCodeAuthData>,
                        }
                        #[derive(Deserialize)]
                        struct ChiCodeAuthData {
                            access_token: String,
                            base_url: String,
                            api_key: String,
                            user: ChiCodeUser,
                        ...
                        let auth_response: ChiCodeAuthResponse = serde_json::from_str(&body)?;
                        if !auth_response.success {
                            return Err(anyhow!("Token exchange failed: {}", auth_response.message));
                        }
                        let auth_data = auth_response.data.ok_or_else(|| anyhow!("No data in auth response"))?;

                        // Log parsed auth data for debugging
                        let api_key_preview = if auth_data.api_key.len() > 15 {
                            format!("{}...{} (len={})", &auth_data.api_key[..8], &auth_data.api_key[auth_data.api_key.len()-8..], auth_data.api_key.len())
                        } else {
                            format!("{} (len={})", &auth_data.api_key, auth_data.api_key.len())
                        };
                        log::info!("[OAuth] Parsed auth data - base_url: {}, api_key: {}", auth_data.base_url, api_key_preview);

                        // Store Chi Code provider credentials for configuring editor providers
                        log::info!("[OAuth] Storing Chi Code provider credentials - base_url: {}", auth_data.base_url);
                        *CHICODE_OAUTH_PROVIDER_CREDS.lock() = Some(ChiCodeProviderCredentials {
                            base_url: auth_data.base_url,
                            api_key: auth_data.api_key,
                        });

                        Ok(Credentials {
                            user_id: auth_data.user.id as u64,
