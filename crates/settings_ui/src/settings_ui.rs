        ...
        .add_renderer::<UnimplementedSettingField>(
            |settings_window, item, _, settings_file, _, sub_field, _, cx| {
                render_settings_item(
                    ...
                    item,
                    settings_file,
                    Button::new("open-in-settings-file", t("settings-edit-in-json"))
                        .style(ButtonStyle::Outlined)
                        .size(ButtonSize::Medium)
                        .tab_index(0_isize)
                        .tooltip(Tooltip::text(t("settings-edit-in-json")))
        ...
        cx.open_window(
            WindowOptions {
                titlebar: Some(TitlebarOptions {
                    title: Some(t("settings-window-title").into()),
...
impl SettingsPageItem {
    fn header_text(&self) -> Option<&str> {
        match self {
            SettingsPageItem::SectionHeader(header) => Some(header.as_ref()),
            _ => None,
        }
    }
    ...
    fn render(
        ...
        match self {
            SettingsPageItem::SectionHeader(header) => {
                SettingsSectionHeader::new(SharedString::from(header.to_string())).into_any_element()
            }
            SettingsPageItem::SettingItem(setting_item) => {
                                    ...
                                    let header_text = this
                                        ...
                                        .or_else(|| {
                                            this.current_page()
                                                ...
                                                .rev()
                                                .find_map(|item| {
                                                    item.header_text().map(|h| SharedString::from(h.to_string()))
                                                })
                                        });
...
fn render_settings_item(
    ...
    h_flex()
        .id(setting_item.title.to_string())
        .min_w_0()
        .justify_between()
        .child(
            v_flex()
                ...
                .child(
                    h_flex()
                        .w_full()
                        .gap_1()
                        .child(Label::new(SharedString::from(setting_item.title.clone())))
                        .when_some(
                            if sub_field {
                                None
                            } else {
                            ...
                            |this, reset_to_default| {
                                this.child(
                                    IconButton::new("reset-to-default-btn", IconName::Undo)
                                        .icon_color(Color::Muted)
                                        .icon_size(IconSize::Small)
                                        .tooltip(Tooltip::text(t("settings-reset-to-default")))
                                        .on_click({
                                            move |_, window, cx| {
                ...
                .child(
                    Label::new(SharedString::from(setting_item.description.clone()))
                        .size(LabelSize::Small)
                        .color(Color::Muted),
                ),
        )
        ...
        .when(settings_window.sub_page_stack.is_empty(), |this| {
            this.child(render_settings_item_link(
                setting_item.description.to_string(),
    ...
    div()
        ...
        .child(
            IconButton::new((id.into(), "copy-link-btn"), link_icon)
                ...
                .tooltip(Tooltip::text(t("settings-copy-link")))
...
impl SettingsUiFile {
    fn setting_type(&self) -> String {
        match self {
            SettingsUiFile::User => t("settings-file-user"),
            SettingsUiFile::Project(_) => t("settings-file-project"),
            SettingsUiFile::Server(_) => t("settings-file-server"),
        }
    }
...
impl SettingsWindow {
    fn new(
        ...
        let search_bar = cx.new(|cx| {
            ...
            editor.set_placeholder_text(&t("settings-search-placeholder"), window, cx);
            editor
        });
    ...
    fn build_search_index(&mut self) {
        ...
        for (page_index, page) in self.pages.iter().enumerate() {
            ...
            for (item_index, item) in page.items.iter().enumerate() {
                ...
                match item {
                    ...
                    | SettingsPageItem::SettingItem(item) => {
                        ...
                        documents.push(bm25::Document {
                            id: key_index,
                            contents: [page.title.as_ref(), header_str, item.title.as_ref(), item.description.as_ref()]
                                .join("\n"),
                        });
                        push_candidates(&mut fuzzy_match_candidates, key_index, item.title.as_ref());
                        push_candidates(&mut fuzzy_match_candidates, key_index, item.description.as_ref());
                    }
                    SettingsPageItem::SectionHeader(header) => {
                        documents.push(bm25::Document {
                            id: key_index,
                    ...
                    SettingsPageItem::SubPageLink(sub_page_link) => {
                        ...
                        documents.push(bm25::Document {
                            id: key_index,
                            contents: [page.title.as_ref(), header_str, sub_page_link.title.as_ref()]
                                .join("\n"),
                        });
                        push_candidates(
                    ...
                    SettingsPageItem::ActionLink(action_link) => {
                        documents.push(bm25::Document {
                            id: key_index,
                            contents: [page.title.as_ref(), header_str, action_link.title.as_ref()]
                                .join("\n"),
                        });
                        push_candidates(
                            &mut fuzzy_match_candidates,
                ...
                push_candidates(&mut fuzzy_match_candidates, key_index, page.title.as_ref());
    ...
    fn render_files_header(
        ...
        h_flex()
                        ...
                        div.child(file_button(selected_file_ix, file, focus_handle, cx))
                            .when(self.files.len() > OVERFLOW_LIMIT + 1, |div| {
                                div.child(
                                    ...
                                    .trigger_tooltip(Tooltip::text(t("settings-view-other-projects")))
            ...
            .child(
                Button::new(edit_in_json_id, t("settings-edit-in-json"))
                    .tab_index(0_isize)
                    .style(ButtonStyle::OutlinedGhost)
                    .tooltip(Tooltip::text(t("settings-edit-in-json")))
                    .on_click(cx.listener(|this, _, window, cx| {
                                ...
                                this.visible_navbar_entries()
                                    ...
                                    .map(|(entry_index, entry)| {
                                        ...
                                        .on_click({
                                            ...
                                            let subcategory =
                                                (!entry.is_root).then(|| entry.title.clone());
    ...
    fn render_sub_page_breadcrumbs(&self) -> impl IntoElement {
        h_flex().gap_1().children(
            itertools::intersperse(
                std::iter::once(SharedString::from(self.current_page().title.to_string())).chain(
                    self.sub_page_stack
                        .iter()
                        .enumerate()
                        .flat_map(|(index, page)| {
    ...
    fn render_no_results(&self, cx: &App) -> impl IntoElement {
        ...
        v_flex()
            ...
            .justify_center()
            .gap_1()
            .child(Label::new(t("settings-no-results")))
            .child(
                Label::new(t("settings-no-match").replace("{query}", &search_query))
                    .size(LabelSize::Small)
                    .color(Color::Muted),
            )
    }
            ...
            SettingsStore::global(cx).error_for_file(self.current_file.to_settings())
        {
            fn banner(
                ...
                Banner::new()
                    ...
                    .action_slot(
                        div().pr_1().pb_1().child(
                            Button::new("fix-in-json", t("settings-fix-in-json"))
                                .tab_index(0_isize)
