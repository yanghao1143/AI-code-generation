//! Paths to locations used by Chi Code.
...
/// On macOS, this is `~/Library/Application Support/ChiCode`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/chicode`.
/// On Windows, this is `%LOCALAPPDATA%\ChiCode`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/chicode`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/chicode`.
/// On Windows, this is `%APPDATA%\ChiCode`.
...
/// Returns the path to the configuration directory used by Chi Code.
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            home_dir().join(".config").join("chicode")
        }
    })
}

/// Returns the path to the data directory used by Chi Code.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
        } else if cfg!(target_os = "macos") {
            home_dir().join("Library/Application Support/ChiCode")
        } else if cfg!(any(target_os = "linux", target_os = "freebsd")) {
...
/// Returns the path to the temp directory used by Chi Code.
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("chicode")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/ChiCode")
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the Chi Code server directory on this SSH host.
...
/// Returns the path to the `ChiCode.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("ChiCode.log"))
}

/// Returns the path to the `ChiCode.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("ChiCode.log.old"))
}
