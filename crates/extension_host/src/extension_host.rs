...
impl ExtensionStore {
    ...
    fn fetch_extensions_from_api(
        ...
        // Chi Code: 优先从我们的 GitHub 仓库获取扩展列表
        if path == "/extensions" {
            return self.fetch_extensions_from_chicode(cx);
        }
    ...
    /// Chi Code: 从 GitHub 仓库获取扩展列表
    fn fetch_extensions_from_chicode(
        &self,
        cx: &mut Context<ExtensionStore>,
    ) -> Task<Result<Vec<ExtensionMetadata>>> {
        let http_client = self.http_client.clone();
        let index_url = chicode_registry::extensions_index_url();
        log::info!("Fetching Chi Code extensions from: {}", index_url);

        cx.spawn(async move |_, _| {
            let mut response = http_client
                .get(&index_url, AsyncBody::empty(), true)
                .await
                .context("failed to fetch Chi Code extension index")?;
            ...
            response
                ...
                .read_to_end(&mut body)
                .await
                .context("error reading extension index")?;

            if response.status().is_client_error() {
                let text = String::from_utf8_lossy(body.as_slice());
                bail!(
                    "Chi Code extension index error {}, response: {text:?}",
                    response.status().as_u16()
                );
            }
            ...
            struct ChiCodeIndex {
                extensions: Vec<ChiCodeExtension>,
            }

            #[derive(serde::Deserialize)]
            struct ChiCodeExtension {
                id: String,
                #[serde(default)]
                name: String,
                #[serde(default)]
            ...
            let index: ChiCodeIndex = serde_json::from_slice(&body)
                .context("failed to parse Chi Code extension index")?;

            // 转换为 ExtensionMetadata 格式
            let extensions: Vec<ExtensionMetadata> = index
                .extensions
                .into_iter()
                .filter(|ext| !SUPPRESSED_EXTENSIONS.contains(&ext.id.as_str()))
                .map(|ext| {
                    let name = if ext.name.is_empty() { ext.id.clone() } else { ext.name };
                    let description = if ext.description.is_empty() {
                        format!("Chi Code 扩展: {}", name)
                    } else {
                        ext.description
                    };
                    ExtensionMetadata {
                        id: ext.id.clone().into(),
                        manifest: client::ExtensionApiManifest {
                            name,
                            version: "latest".into(),
                        ...
                        download_count: if ext.priority { 1000 } else { 0 },
                    }
                })
                .collect();
    ...
    pub fn install_latest_extension(&mut self, extension_id: Arc<str>, cx: &mut Context<Self>) {
        ...
        // Chi Code: 使用 GitHub 仓库的下载地址
        let download_url = chicode_registry::extension_download_url(&extension_id);
        let Ok(url) = Url::parse(&download_url) else {
            log::error!("failed to parse Chi Code extension download URL: {}", download_url);
            return;
        };
    ...
    fn install_or_upgrade_extension(
        ...
        // Chi Code: 使用 GitHub 仓库的下载地址
        let download_url = chicode_registry::extension_version_download_url(&extension_id, &version);
        let Ok(url) = Url::parse(&download_url) else {
            log::error!("failed to parse Chi Code extension download URL: {}", download_url);
