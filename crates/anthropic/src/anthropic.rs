...
async fn send_request(
    ...
    _beta_headers: Option<String>,
) -> Result<(http::Response<AsyncBody>, RateLimitInfo), AnthropicError> {
    let uri = format!("{api_url}/v1/messages");

    // Debug: print actual URI and request body
    let request_json = serde_json::to_string(&request).unwrap_or_default();
    eprintln!("[Anthropic DEBUG] URI: {}", uri);
    // Print first 1500 chars to check if temperature/top_p are in the request
    eprintln!("[Anthropic DEBUG] Request body (first 1500 chars): {}", &request_json[..request_json.len().min(1500)]);
    // Check if temperature or top_p are in the JSON
    if request_json.contains("temperature") {
        eprintln!("[Anthropic DEBUG] WARNING: Request contains 'temperature' field!");
    }
    if request_json.contains("top_p") {
        eprintln!("[Anthropic DEBUG] WARNING: Request contains 'top_p' field!");
    }

    // Detect OS and architecture for x-stainless headers
    ...
    // Build request with exact Claude Code CLI headers
    let request_builder = HttpRequest::builder()
        .method(Method::POST)
        .uri(uri)
        // Auth - only X-Api-Key (Claude Code CLI style)
        .header("X-Api-Key", api_key.trim())
        // Anthropic required headers
        .header("Anthropic-Version", "2023-06-01")
        // Client identification - Claude Code CLI markers
        .header("User-Agent", "claude-cli/2.1.27")
        .header("x-claude-cli-version", "2.1.27")
        // Content headers
    ...
    eprintln!("[Anthropic DEBUG] Request headers:");
    ...
    eprintln!("[Anthropic DEBUG] Response status: {}", response.status());
    eprintln!("[Anthropic DEBUG] Response headers:");
...
async fn handle_error_response(
    ...
    eprintln!("[Anthropic DEBUG] Error response body: {}", &body);
