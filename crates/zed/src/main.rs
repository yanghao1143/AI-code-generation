...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Chi Code failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Chi Code failed to open a window: {e:?}. See https://vip.chiddns.com/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "com.chicode.ChiCode.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Chi Code failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://vip.chiddns.com/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    if args.system_specs {
        ...
        println!("Chi Code System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    app.run(move |cx| {
        ...
        if let Err(e) = i18n::set_locale(&system_locale.to_string()) {
            log::warn!("Failed to set locale {}: {:?}", system_locale, e);
        }
        log::info!("Locale initialized: {}", i18n::current_locale());
...
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/ChiCode`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/chicode`.
    /// On Windows, the default is `%LOCALAPPDATA%\ChiCode`.
    ...
    /// Instructs chicode to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents Chi Code from starting, so you can't run `chicode: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,
    ...
    /// Used for recording minidumps on crashes by having Chi Code run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run chicode in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
