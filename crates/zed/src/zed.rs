...
fn unstable_version_notification(cx: &mut App) {
    ...
    show_app_notification(NotificationId::unique::<WindowsNightly>(), cx, |cx| {
        cx.new(|cx| {
            MessageNotification::new("You're using an unstable version of Chi Code (Nightly)", cx)
                ...
                .primary_on_click(|window, cx| {
                    window.dispatch_action(
                        zed_actions::OpenBrowser {
                            url: "https://vip.chiddns.com/download".to_string(),
                        }
                        .boxed_clone(),
                        cx,
                    );
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://vip.chiddns.com/docs/linux
            "#},
            e
        );
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://vip.chiddns.com/docs/linux#could-not-start-inotify");
                    cx.quit();
                });
            }
        })
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: https://vip.chiddns.com/docs/windows
            "#},
            e
        );
        let prompt = window.prompt(
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://vip.chiddns.com/docs/windows");
                    cx.quit()
                });
            }
        })
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("ZED_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            (
                "DirectX",
                "https://vip.chiddns.com/docs/windows",
                "https://vip.chiddns.com/docs/windows",
            )
        } else {
            (
                "Vulkan",
                "https://vip.chiddns.com/docs/linux",
                "https://vip.chiddns.com/docs/linux#zed-fails-to-open-windows",
            )
        };
        let message = format!(
            db::indoc! {r#"
            Chi Code uses {} for rendering and requires a compatible GPU.
...
fn initialize_panels(
    ...
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        let cc_switch_panel = CcSwitchPanel::load(workspace_handle.clone(), cx.clone());
        ...
        futures::join!(
            ...
            add_panel_when_ready(cc_switch_panel, workspace_handle.clone(), cx.clone()),
