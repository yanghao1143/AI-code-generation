...
fn render_theme_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    return v_flex()
        .gap_2()
        .child(
            h_flex().justify_between().child(Label::new(t("welcome-theme"))).child(
                ToggleButtonGroup::single_row(
                    ...
                    .map(|mode| {
                        let mode_names: [String; 3] = [
                            t("welcome-theme-light"),
                            t("welcome-theme-dark"),
                            t("welcome-theme-system"),
                        ];
    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(t("telemetry-metrics-desc").into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(t("telemetry-diagnostics-desc").into()),
...
fn render_base_keymap_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    return v_flex().gap_2().child(Label::new(t("welcome-keymap"))).child(
...
fn render_vim_mode_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    SwitchField::new(
        "onboarding-vim-mode",
        Some(t("welcome-vim-mode")),
        Some(t("welcome-vim-mode-desc").into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = t("trust-tooltip-desc");

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some(t("trust-all-projects")),
        Some(t("trust-all-projects-desc").into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
    ...
    h_flex()
        ...
        .child(
            v_flex()
                ...
                .child(
                    Label::new(t("import-settings-desc"))
                        .color(Color::Muted),
                ),
        )
