...
impl settings::Settings for AllLanguageModelSettings {
    ...
    fn from_settings(content: &settings::SettingsContent) -> Self {
        ...
        log::info!("[Settings] Loading anthropic settings - api_url: {:?}, api_key is_some: {}",
            anthropic.api_url, anthropic.api_key.is_some());
        if let Some(ref k) = anthropic.api_key {
            log::info!("[Settings] anthropic.api_key value: {}... (len={})", &k[..5.min(k.len())], k.len());
        }
        ...
        Self {
            anthropic: AnthropicSettings {
                ...
                use_cli: anthropic.use_cli.unwrap_or(false),
