                ...
                v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .children(
                                Some(
                                    Button::new("copy-link", t("collab-copy-link"))
                        ...
                        h_flex()
                            .child(
                                div()
                                    ...
                                    .child(Label::new(t("collab-manage-members")))
                            ...
                            .child(
                                div()
                                    ...
                                    .child(Label::new(t("collab-invite-members")))
                                    .on_click(cx.listener(|this, _, window, cx| {
...
impl PickerDelegate for ChannelModalDelegate {
    ...
    fn placeholder_text(&self, _window: &mut Window, _cx: &mut App) -> Arc<str> {
        t("collab-search-collaborator").into()
    }
                ...
                .end_slot(h_flex().gap_2().map(|slot| {
                    match self.mode {
                        Mode::ManageMembers => slot
                            .children(
                                if request_status == Some(proto::channel_member::Kind::Invitee) {
                                    Some(Label::new(t("collab-invited")))
                                } else {
                                    None
                                },
                            )
                            .children(match membership.map(|m| m.role) {
                                Some(ChannelRole::Admin) => Some(Label::new(t("collab-admin"))),
                                Some(ChannelRole::Guest) => Some(Label::new(t("collab-guest"))),
                                _ => None,
                            })
                            .when(!is_me, |el| {
                                el.child(IconButton::new("ellipsis", IconName::Ellipsis))
                            })
                            .when(is_me, |el| el.child(Label::new(t("collab-you")).color(Color::Muted)))
                            .children(
                        ...
                        Mode::InviteMembers => match request_status {
                            Some(proto::channel_member::Kind::Invitee) => {
                                slot.children(Some(Label::new(t("collab-invited"))))
                            }
                            Some(proto::channel_member::Kind::Member) => {
                                slot.children(Some(Label::new(t("collab-member"))))
                            }
                            _ => slot,
                        },
                    }
    ...
    fn show_context_menu(
        ...
        let context_menu = ContextMenu::build(window, cx, |mut menu, _window, _cx| {
            ...
            if role == ChannelRole::Admin || role == ChannelRole::Member {
                ...
                menu = menu.entry(t("collab-demote-to-guest"), None, move |window, cx| {
                    picker.update(cx, |picker, cx| {
                        picker
                            .delegate
            ...
            if role == ChannelRole::Member || role == ChannelRole::Guest {
                let picker = picker.clone();
                menu = menu.entry(t("collab-promote-to-admin"), None, move |window, cx| {
                    picker.update(cx, |picker, cx| {
                        picker
                            .delegate
            ...
            menu = menu.separator();
            menu = menu.entry(t("collab-remove-from-channel"), None, {
                let picker = picker.clone();
                move |window, cx| {
                    picker.update(cx, |picker, cx| {
