...
impl CollabPanel {
    pub fn new(
        ...
        cx.new(|cx| {
            let filter_editor = cx.new(|cx| {
                ...
                editor.set_placeholder_text(&t("collab-search-channels"), window, cx);
                editor
            });
    ...
    fn render_call_participant(
        ...
        let tooltip = t_args("collab-follow-user", &[("name", user.github_login.as_ref())].into_iter().collect());
        ...
        ListItem::new(user.github_login.clone())
            ...
            .end_slot(if is_pending {
                Label::new(t("collab-calling")).color(Color::Muted).into_any_element()
            } else if is_current_user {
                IconButton::new("leave-call", IconName::Exit)
                    .style(ButtonStyle::Subtle)
                    .on_click(move |_, window, cx| Self::leave_call(window, cx))
                    .tooltip(Tooltip::text(t("collab-leave-call")))
                    .into_any_element()
            } else if role == proto::ChannelRole::Guest {
                Label::new(t("collab-guest")).color(Color::Muted).into_any_element()
            } else if role == proto::ChannelRole::Talker {
                Label::new(t("collab-mic-only"))
                    .color(Color::Muted)
                    .into_any_element()
            } else {
    ...
    fn render_participant_project(
        ...
        ListItem::new(project_id as usize)
            ...
            .tooltip(Tooltip::text(t_args("collab-open-project", &[("name", project_name.as_ref())].into_iter().collect())))
    }
    ...
    fn render_participant_screen(
        ...
        ListItem::new(("screen", id))
            ...
            .child(Label::new(t("collab-screen")))
            .when_some(peer_id, |this, _| {
                ...
                .tooltip(Tooltip::text(t("collab-open-shared-screen")))
            })
    }
    ...
    fn render_channel_notes(
        ...
        ListItem::new("channel-notes")
            ...
            .child(Label::new(t("collab-notes")))
            .tooltip(Tooltip::text(t("collab-open-channel-notes")))
    }
        ...
        let context_menu = ContextMenu::build(window, cx, |mut context_menu, window, cx| {
            if self.has_subchannels(ix) {
                let expand_action_name = if self.is_channel_collapsed(channel_id) {
                    t("collab-expand-subchannels")
                } else {
                    t("collab-collapse-subchannels")
                };
                context_menu = context_menu.entry(
                    expand_action_name,
                    None,
            ...
            context_menu = context_menu
                ...
                .entry(
                    t("collab-copy-channel-notes-link"),
                    None,
                    window.handler_for(&this, move |this, _, cx| {
            ...
            if self.channel_store.read(cx).is_channel_admin(channel_id) {
                ...
                if let Some(channel_name) = clipboard_channel_name {
                    context_menu = context_menu.separator().entry(
                        t_args("collab-move-channel-here", &[("name", channel_name.as_ref())].into_iter().collect()),
                        None,
                        window.handler_for(&this, move |this, window, cx| {
        ...
        let context_menu = ContextMenu::build(window, cx, |mut context_menu, _, _| {
            ...
            if contact.online && !contact.busy {
                let label = if in_room {
                    t_args("collab-invite-to-join", &[("name", contact.user.github_login.as_ref())].into_iter().collect())
                } else {
                    t_args("collab-call-user", &[("name", contact.user.github_login.as_ref())].into_iter().collect())
                };
            ...
            context_menu.entry(t("collab-remove-contact"), None, {
                let this = this.clone();
                move |window, cx| {
                    this.update(cx, |this, cx| {
                        this.remove_contact(
    ...
    fn leave_channel(&self, channel_id: ChannelId, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let prompt_message = t_args("collab-confirm-leave-channel", &[("name", channel.name.as_ref())].into_iter().collect());
        let leave_label = t("collab-leave");
        let cancel_label = t("cancel");
        let answer = window.prompt(
            PromptLevel::Warning,
            &prompt_message,
            None,
            &[leave_label.as_str(), cancel_label.as_str()],
            cx,
        );
        cx.spawn_in(window, async move |this, cx| {
            if answer.await? != 0 {
    ...
    fn remove_channel(
        ...
        if let Some(channel) = channel_store.read(cx).channel_for_id(channel_id) {
            let prompt_message = t_args("collab-confirm-remove-channel", &[("name", channel.name.as_ref())].into_iter().collect());
            let remove_label = t("collab-remove");
            let cancel_label = t("cancel");
            let answer = window.prompt(
                PromptLevel::Warning,
                &prompt_message,
                None,
                &[remove_label.as_str(), cancel_label.as_str()],
                cx,
            );
            cx.spawn_in(window, async move |this, cx| {
                if answer.await? == 0 {
    ...
    fn remove_contact(
        ...
        let user_store = self.user_store.clone();
        let prompt_message = t_args("collab-confirm-remove-contact", &[("name", github_login)].into_iter().collect());
        let remove_label = t("collab-remove");
        let cancel_label = t("cancel");
        let answer = window.prompt(
            PromptLevel::Warning,
        ...
        v_flex()
            ...
            .child(
                v_flex()
                    .gap_2()
                    .child(
                        Button::new("sign_in", t("collab-sign-in"))
                    ...
                    .child(
                        v_flex().w_full().items_center().child(
                            Label::new(t("collab-sign-in-required"))
                                .color(Color::Muted)
    ...
    fn render_contact(
        ...
        let item = ListItem::new(github_login.clone())
            ...
            .child(
                h_flex()
                    ...
                    .when(calling, |el| {
                        el.child(Label::new(t("collab-calling")).color(Color::Muted))
                    })
                    .when(!calling, |el| {
                        el.child(
        ...
        div()
            ...
            .tooltip(move |_, cx| {
                let text = if !online {
                    t_args("collab-user-offline", &[("name", github_login.as_ref())].into_iter().collect())
                } else if busy {
                    t_args("collab-user-on-call", &[("name", github_login.as_ref())].into_iter().collect())
                } else {
                    let room = ActiveCall::global(cx).read(cx).room();
                    if room.is_some() {
                        t_args("collab-invite-to-call", &[("name", github_login.as_ref())].into_iter().collect())
                    } else {
                        t_args("collab-call-user", &[("name", github_login.as_ref())].into_iter().collect())
                    }
                };
        ...
        let controls = if is_incoming {
            ...
            vec![
                IconButton::new("remove_contact", IconName::Close)
                    ...
                    .icon_color(color)
                    .tooltip(Tooltip::text(t("collab-cancel-invite"))),
            ]
        };
    ...
    fn render_contact_placeholder(&self, is_selected: bool, cx: &mut Context<Self>) -> ListItem {
        ListItem::new("contact-placeholder")
            ...
            .child(Label::new(t("collab-add-contact")))
            .toggle_state(is_selected)
...
impl Render for JoinChannelTooltip {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        tooltip_container(cx, |container, cx| {
            ...
            container
                .child(Label::new(t("collab-join-channel")))
