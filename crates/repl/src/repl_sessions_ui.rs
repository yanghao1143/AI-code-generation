...
impl Render for ReplSessionsPage {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if kernel_specifications.is_empty() {
            let instructions = t("repl-no-kernels-instructions");

            return ReplSessionsContainer::new(t("repl-no-kernels-available"))
                ...
                .child(
                    h_flex().w_full().p_4().justify_center().gap_2().child(
                        ButtonLike::new("install-kernels")
                            ...
                            .child(Label::new(t("repl-install-kernels")))
        ...
        if sessions.is_empty() {
            let instructions = t("repl-no-sessions-instructions");

            return ReplSessionsContainer::new(t("repl-no-sessions")).child(
                v_flex()
                    .child(Label::new(instructions))
        ...
        ReplSessionsContainer::new(t("repl-kernel-sessions")).children(sessions)
    }
}
