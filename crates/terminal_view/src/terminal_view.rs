    ...
    pub fn deploy_context_menu(
        ...
        let context_menu = ContextMenu::build(window, cx, |menu, _, _| {
            menu.context(self.focus_handle.clone())
                .action(t("terminal-new-terminal"), Box::new(NewTerminal::default()))
                .separator()
                .action(t("terminal-copy"), Box::new(Copy))
                .action(t("terminal-paste"), Box::new(Paste))
                .action(t("terminal-select-all"), Box::new(SelectAll))
                .action(t("terminal-clear"), Box::new(Clear))
                .when(assistant_enabled, |menu| {
                    menu.separator()
                        .action(t("terminal-inline-assist"), Box::new(InlineAssist::default()))
                })
                .separator()
                .action(
                    t("terminal-close-tab"),
    ...
    fn rerun_button(task: &TaskState) -> Option<IconButton> {
        ...
        Some(
            IconButton::new("rerun-icon", IconName::Rerun)
                ...
                .tooltip(move |_window, cx| Tooltip::for_action(t("terminal-rerun-task"), &RerunTask, cx))
