...
#[command(
    ...
    before_help = "The Chi Code CLI binary.
This CLI is a separate binary that invokes Chi Code.

Examples:
    `chicode`
          Simply opens Chi Code
    `chicode --foreground`
          Runs in foreground (shows all logs)
    `chicode path-to-your-project`
          Open your project in Chi Code
    `chicode -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | chicode -'"
)]
struct Args {
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/ChiCode`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\ChiCode`.")]
    #[cfg_attr(
        not(any(target_os = "windows", target_os = "macos")),
        doc = "`$XDG_DATA_HOME/chicode`."
    )]
    #[arg(long, value_name = "DIR")]
    user_data_dir: Option<String>,
    /// The paths to open in Chi Code (space-separated).
    ///
    /// Use `path:line:column` syntax to open a file at the given line and column.
    paths_with_position: Vec<String>,
    /// Print Chi Code's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run chicode in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to ChiCode.app or the chicode binary
    #[arg(long)]
    zed: Option<PathBuf>,
    /// Run chicode in dev-server mode
    #[arg(long)]
    dev_server_token: Option<String>,
    /// The username and WSL distribution to use when opening paths. If not specified,
    /// Chi Code will attempt to open the paths directly.
    ///
    ...
    wsl: Option<String>,
    /// Not supported in Chi Code CLI, only supported on Chi Code binary
    ...
    diff: Vec<String>,
    /// Uninstall Chi Code from user system
    #[cfg(all(
    ...
    /// by having Chi Code act like netcat communicating over a Unix socket.
    #[arg(long, hide = true)]
...
fn main() -> Result<()> {
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the Chi Code CLI, only on Chi Code binary.",
...
mod flatpak {
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").is_ok_and(|id| id.starts_with("com.chicode.ChiCode"))
            && args.zed.is_none()
        {
            args.zed = Some("/app/libexec/zed-editor".into());
            unsafe { env::set_var("ZED_UPDATE_EXPLANATION", "Please use flatpak to update chicode") };
        }
        args
    }
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("com.chicode.ChiCode") {
                return None;
            }
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("Chi Code {} – {}", self.version(), self.path().display(),)
        }
