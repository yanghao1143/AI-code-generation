                ...
                menu.context(self.focus_handle.clone()).map(|menu| {
                    if is_read_only {
                        menu.when(is_dir, |menu| {
                            menu.action(t("project-search-inside"), Box::new(NewSearchInDirectory))
                        })
                    } else {
                        menu.action(t("project-new-file"), Box::new(NewFile))
                            .action(t("project-new-folder"), Box::new(NewDirectory))
                            .separator()
                            .when(is_local, |menu| {
                                menu.action(
                                    if cfg!(target_os = "macos") && !is_remote {
                                        t("project-reveal-finder")
                                    } else if cfg!(target_os = "windows") && !is_remote {
                                        t("project-reveal-explorer")
                                    } else {
                                        t("project-reveal-file-manager")
                                    },
                                    Box::new(RevealInFileManager),
                                )
                            })
                            .when(is_local, |menu| {
                                menu.action(t("project-open-default-app"), Box::new(OpenWithSystem))
                            })
                            .action(t("project-open-terminal"), Box::new(OpenInTerminal))
                            .when(is_dir, |menu| {
                                menu.separator()
                                    .action(t("project-find-in-folder"), Box::new(NewSearchInDirectory))
                            })
                            .when(is_unfoldable, |menu| {
                                menu.action(t("project-unfold-directory"), Box::new(UnfoldDirectory))
                            })
                            .when(is_foldable, |menu| {
                                menu.action(t("project-fold-directory"), Box::new(FoldDirectory))
                            })
                            .when(should_show_compare, |menu| {
                                menu.separator()
                                    .action(t("project-compare-marked"), Box::new(CompareMarkedFiles))
                            })
                            .separator()
                            .action(t("project-cut"), Box::new(Cut))
                            .action(t("project-copy"), Box::new(Copy))
                            .action(t("project-duplicate"), Box::new(Duplicate))
                            ...
                            .separator()
                            .action(t("project-copy-path"), Box::new(zed_actions::workspace::CopyPath))
                            .action(
                                t("project-copy-relative-path"),
                                Box::new(zed_actions::workspace::CopyRelativePath),
                            )
                            .when(!is_dir && self.has_git_changes(entry_id), |menu| {
                                menu.separator().action(
                                    t("project-restore-file"),
                                    Box::new(git::RestoreFile { skip_prompt: false }),
                                )
                            })
                            .when(has_git_repo, |menu| {
                                menu.separator()
                                    .action(t("project-view-file-history"), Box::new(git::FileHistory))
                            })
                            .when(!should_hide_rename, |menu| {
                                menu.separator().action(t("project-rename"), Box::new(Rename))
                            })
                            .when(!is_root && !is_remote, |menu| {
                                menu.action(t("project-trash"), Box::new(Trash { skip_prompt: false }))
                            })
                            .when(!is_root, |menu| {
                                menu.action(t("project-delete"), Box::new(Delete { skip_prompt: false }))
                            })
                            .when(!is_collab && is_root, |menu| {
                                menu.separator()
                                    .action(
                                        t("project-add-folder"),
                                        Box::new(workspace::AddFolderToProject),
                                    )
                                    .action(t("project-remove-from"), Box::new(RemoveFromProject))
                            })
                            .when(is_root, |menu| {
                                menu.separator()
                                    .action(t("project-collapse-all"), Box::new(CollapseAllEntries))
                            })
                    }
                })
            });
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if has_worktree {
            ...
            v_flex()
                ...
                .track_focus(&self.focus_handle(cx))
                .child(
                    Button::new("open_project", t("project-open"))
                        .full_width()
                ...
                .child(
                    h_flex()
                        ...
                        .child(Label::new(t("or")).size(LabelSize::XSmall).color(Color::Muted))
                        .child(Divider::horizontal()),
                )
                .child(
                    Button::new("clone_repo", t("project-clone-repo"))
                        .full_width()
                        .on_click(cx.listener(|this, _, window, cx| {
                            this.workspace
                                .update(cx, |_, cx| {
